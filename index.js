...
console.log(color(figlet.textSync('GansBOTz', {
	...
	console.log(color(`${spc2}           [ Created by: Deffri Gans ]` ,`${randomcolor}`))
	...
	console.log(color(`${spc3}                    [•]`, `${randomcolor}`), color(`Author      : Deffri Gans`, `${randomcolor}`))
	console.log(color(`${spc3}                       < =============================== >`, `${randomcolor}`))
	let { version, isLatest } = await fetchLatestBaileysVersion()
    const gans = WaConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['GansBOTz','Chrome','5.9.9'],
        auth: state,
        version
    })
